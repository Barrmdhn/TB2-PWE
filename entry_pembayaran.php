	...
	<div class="content">
		...
		<form action="insertpembayaran.php" method="POST">
			<div class="input-group">
				<input type="text" placeholder="Nomor Indentitas Pembayaran" value="0000<?php while($rows=mysqli_fetch_array($hasil)){ $num = $rows['no_pembayaran']+$no; echo $num; }?>" name="no_pembayaran">
			</div>
			<div class="input-group">
				<input type="text" placeholder="Nomor Tiket" name="no_tiket" value="DM0000<?php while($rows=mysqli_fetch_array($hasil2)){ $dat = $rows['no_tiket']; $num = substr($dat,5)+$no; echo $num; }?>" >
			</div>
			<div class="input-group">
